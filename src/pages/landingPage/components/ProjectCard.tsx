...
function ProjectCard({ cardImageUrl, title, description, tags, cb }: ProjectCardProps) {
    return (
        <button
            type="button"
            className="transition ease-in-out delay-150 flex flex-col w-[200px] h-[300px] bg-stone-100 shadow-sm rounded-lg hover:scale-110"
            onClick={cb}
        >
                    ...
                    {_.map(tags, (tag) => {
                        return (
                            <span
                                key={tag.name}
                                className={cls(
                                    'text-[10px] px-[6px] mx-1 rounded-sm shadow-lg',
