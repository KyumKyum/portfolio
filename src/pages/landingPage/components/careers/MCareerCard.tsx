...
function MCareerCard({ cardImageUrl, title, description, details }: CareerCardProps) {
    return (
        <div className="flex w-[340px] h-[400px] bg-stone-100 shadow-sm rounded-lg">
                        ...
                        _.map(details, (detail) => {
                            return (
                                <ul key={detail.title} className="mb-2">
                                    <li className="text-[12px] mx-1 font-Suit font-bold text-black">{detail.title}</li>
                                    {_.map(detail.descriptions, (description, index) => {
                                        return <li key={index} className="text-[11px] font-Suit text-slate-800">- {description}</li>;
                                    })}
                                </ul>
                            );
                        })}
